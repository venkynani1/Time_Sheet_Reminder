...
function Settings() {
  ...
  async function testEmail(event) {
    ...
    try {
      setNotice({ type: 'success', text: (await sendTestEmail(email)).message });
      setDiagnostics(await getEmailDiagnostics());
    }
    catch (error) { setNotice({ type: 'danger', text: error.response?.data?.error || 'Could not send the simple email test.' }); }
  ...
  async function testReminderTemplate(event) {
    ...
    try {
      const result = await sendReminderTemplateTest(email);
  ...
  return <>
    ...
    <div className="card content-card mt-4"><div className="card-body p-4">
      <div className="d-flex flex-wrap justify-content-between gap-3 mb-3"><div><h2 className="h5">Email Diagnostics</h2><p className="text-secondary mb-0">Compare a simple Gmail API send with the current reminder template send.</p></div><button className="btn btn-outline-secondary" onClick={() => getEmailDiagnostics().then(setDiagnostics)} type="button">Refresh</button></div>
      ...
      <form className="d-flex flex-wrap gap-2 mb-4"><input className="form-control flex-grow-1" onChange={(event) => setEmail(event.target.value)} placeholder="recipient@example.com" type="email" value={email} /><button className="btn btn-primary" disabled={sending} onClick={testEmail} type="button">{sending ? 'Sending...' : 'Simple Email Test'}</button><button className="btn btn-outline-primary" disabled={sendingTemplateTest} onClick={testReminderTemplate} type="button">{sendingTemplateTest ? 'Sending...' : 'Reminder Template Test'}</button></form>
